...
On PASS, freeze the contract and move the backlog item to `IN_PROGRESS`, or to `REOPENED_FOR_UI` when the draft is explicitly a UI-only reopening contract.

Use `HUMAN_DECISION_REQUIRED` instead of PASS/FAIL when the reviewer finds a product, sequencing, UI mapping, or delivery tradeoff that needs human judgment. In that case, record the issue, source references, options, and recommendation in `working/current-plan-review.md`.
