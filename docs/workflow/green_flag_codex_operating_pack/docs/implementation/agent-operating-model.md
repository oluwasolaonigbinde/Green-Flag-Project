...
Preconditions: exactly one `IN_PROGRESS` slice and a frozen contract.
...
- If the work is UI-only and maps to an existing `DONE_BACKEND` or `DONE_FULL` slice, create a UI-only reopening plan and mark `REOPENED_FOR_UI` after review.
